...
- `src/components/oauth-icons.tsx` — Branded SVG icons for Google + Microsoft. Re-exports `Github` from lucide-react as `GithubIcon`.
- `app/signup/page.tsx` — Server component redirect to `/login?mode=signup`
- `app/api/cleanup/route.ts` — Best-effort anonymous data cleanup via `sendBeacon` (POST). Deletes anonymous documents + storage.
...
- `src/context/stream-context.tsx` — `StreamProvider` + `useStreamContext()`. Owns the SSE subscription at layout level so it survives client-side navigation. Stores raw stream state (clauses, summary, positions, status, errors). `startStream(id)` activates for a new analysis; no-op for same ID. `clearStream()` resets. Shimmer buffering is NOT here — it stays local to AnalysisView.
- `src/trpc/` — Client-side tRPC setup (provider, query client)
- `src/lib/utils.ts` — shadcn/ui utility (`cn` function)
- `src/hooks/use-media-query.ts` — `useMediaQuery(query)` + `useIsDesktop()`. SSR-safe (defaults `false`/mobile-first). Used by `AnalysisView` for desktop/mobile layout branching at `lg` (1024px) breakpoint.
...
| `/login` | Static | Unified auth page with `mode` state (`"signin" \| "signup"`). Email/password + magic link. `BackgroundPaths` renders once (no remount on mode switch). `?mode=signup` initializes signup form. Redirects to `/dashboard` on success. |
| `/signup` | Static | Server component redirect to `/login?mode=signup`. |
...
| `/api/cleanup` | API (POST) | Best-effort anonymous data cleanup via `sendBeacon`. Accepts `{ analysisId }`, deletes document + storage if `userId IS NULL`. `runtime = "nodejs"`. |
| `/api/cron/cleanup` | API (GET) | Vercel Cron auto-deletion. Deletes documents >30 days + anonymous documents >24h (decrypts storagePath → deletes from Storage → CASCADE deletes analyses+clauses). Deletes rate_limits >7 days. Verifies `CRON_SECRET` via `timingSafeEqual`. `runtime = "nodejs"`, `maxDuration = 60`. |
...
| `/dashboard` | Static | Dashboard page. Protected by middleware (redirects to `/login` if unauthenticated). Authenticated users on `/login` or `/signup` are redirected here. `HistoryView` client component with infinite scroll via `analysis.list` tRPC query. |
...
| `SecuritySection` | `security-section.tsx` | 6 equal security feature cards in 3x2 grid (lg:3-col, md:2-col, 1-col mobile). Cards: AES-256 Encryption (amber accent), Never Shared or Sold, Open Source (links to GitHub repo), EU Data Center, Anonymous Analysis, Auto-Delete in 30 Days. Open Source card is a clickable link. Each card wrapped in ScrollReveal with staggered delay. |
| `TextShimmer` | `text-shimmer.tsx` | Gradient text animation (motion library). `prefers-reduced-motion` respected. |
| `UploadZone` | `upload-zone.tsx` | Native HTML5 drag-drop + file input. States: idle, drag-over, uploading (progress bar), processing (dots), error, rejection, rate-limit. Handles POST to `/api/upload`. Includes LanguageSelector below drop zone. Props: `onUploadSuccess?: (analysisId: string) => void` (overrides default redirect), `compact?: boolean` (smaller padding, no security line). Used on landing page (default redirect) and dashboard (callback + compact). |
...
| Component | File | Notes |
|-----------|------|-------|
| `AnalysisView` | `analysis-view.tsx` | Client component. Dual-path: tRPC query for initial state, stream context for live data. Calls `stream.startStream(id)` when analysis is pending/processing — SSE subscription lives in `StreamProvider` (layout level, survives navigation). Local `displayedClauses` state handles shimmer buffering (min 400ms per clause via `shimmerStartTimes` + `pendingResults`). `effectiveAnalyzing` memo = context `analyzingPositions` ∪ buffered positions (visual consistency). `processingStep` derived via `useMemo` from context state. **Desktop (≥ lg):** side-by-side layout with `hoveredClause` (visual only) + `pinnedClause` (click → scroll other panel) + `ConnectingLines`. **Mobile (< lg):** single integrated document with inline clause cards via `renderClauseSlot` callback. `expandedClauses` state (`Set<number>`) tracks expanded/collapsed inline cards (red/yellow auto-expanded, green collapsed, tap to toggle). `CompactClauseBar` renders collapsed state. `useIsDesktop()` hook drives layout branching. Auto-scroll: desktop uses `scrollInContainer` within panels; mobile uses `scrollIntoView` on page. Summary skeleton shown between last clause and summary arrival. `GreenClauseCompact` inline for green clauses (desktop right panel). DB render path also branches on `isDesktop`. Initialized from context on navigate-back (instant display, no reconnection). Auth state detected via `createClient().auth.getUser()` for feature gating. Anonymous users see "Create a free account" banner and sendBeacon cleanup on `visibilitychange`. |
| `AnalysisActions` | `analysis-actions.tsx` | Props: `analysisId`, `isOwner`, `isPublic`, `shareExpiresAt`, `isAuthenticated`. Authenticated owner: share toggle (enables 7-day link + copies URL), stop-sharing button (`Link2Off` icon), Download PDF, Delete (with ConfirmDialog). Non-owner: copy link + Download PDF only. Anonymous (`!isAuthenticated`): Download PDF only + "Sign in to share" CTA. Shows "Shared · Expires {date}" when active. Uses `analysis.toggleShare` mutation. Centered below summary in both DB and streaming paths. |
...
| `TextDocumentPanel` | `text-document-panel.tsx` | Block-level clause highlighting (full-width `<div>` blocks, not inline spans). Paragraph detection, alternating shade for same-risk clauses. Exposes scroll container via `onScrollContainerRef` callback. Optional `renderClauseSlot` callback injects ReactNode inside each clause highlight div as last child (used for mobile inline analysis — `InlineClauseExpanded` dark inset or `InlineClauseFooter` collapsed bar). No auto-scroll on hover (click-to-scroll handled by parent). Risk colors, analyzing shimmer, pending gray, flash overlay. |
...
| Component | File | Notes |
|-----------|------|-------|
| `HistoryView` | `history-view.tsx` | Client component. `trpc.analysis.list.useInfiniteQuery` with cursor-based pagination. Reads `useStreamContext()` for live clause counts on the actively-streaming analysis (instant updates, no polling). `isActiveStream` flag prevents stale detection for context-streamed items. 2s polling interval only runs for processing items NOT covered by the active stream. Auto-invalidates list when `stream.streamDone` transitions to true. Renders list items with risk score, document name, contract type badge, recommendation badge, date. Delete button per item (with ConfirmDialog). Empty state for new users. Loading skeleton. "Load more" button for pagination. |
