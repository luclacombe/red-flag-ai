...
- `app/login/page.tsx` — Unified auth page with `mode` state (signin/signup). `BackgroundPaths` renders once. `?mode=signup` initializes signup form. Uses `Suspense` for `useSearchParams`.
...
| Component | File | Notes |
|-----------|------|-------|
| `AnalysisView` | `analysis-view.tsx` | Client component. Dual-path: tRPC query for initial state, stream context for live data. Calls `stream.startStream(id)` when analysis is pending/processing — SSE subscription lives in `StreamProvider` (layout level, survives navigation). Local `displayedClauses` state handles shimmer buffering (min 400ms per clause via `shimmerStartTimes` + `pendingResults`). `effectiveAnalyzing` memo = context `analyzingPositions` ∪ buffered positions (visual consistency). `processingStep` derived via `useMemo` from context state. Interaction model: `hoveredClause` (visual only, no scroll) + `pinnedClause` (click → scroll other panel). Scroll suppression via `isScrollingRef` (150ms debounce). Auto-scroll follows analysis progress (`skeletonRef`) until `userHasInteracted` (wheel/touchmove). Summary skeleton shown between last clause and summary arrival. `GreenClauseCompact` inline for green clauses. DB render path shows side-by-side layout. Initialized from context on navigate-back (instant display, no reconnection). Auth state detected via `createClient().auth.getUser()` for feature gating (`isAuthenticated` prop on `AnalysisActions`). Anonymous users see "Create a free account" banner and sendBeacon cleanup on `visibilitychange`. |
