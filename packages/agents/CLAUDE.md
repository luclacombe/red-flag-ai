...
- **Combined streaming analysis**: A single `client.messages.stream()` call with 3 tools: `report_safe_clauses` (batch green), `report_clause` (individual red/yellow), and `report_summary`. Two-phase workflow: Claude scans all clauses, batches safe ones via `report_safe_clauses` first (green cards appear instantly), then reports risky clauses individually. `strict: true` guarantees valid JSON — zero parse errors.
- **Tool definitions**: `report_safe_clauses` (safeClauses: Array<{position, category, note}>), `report_clause` (position, riskLevel, explanation, category, saferAlternative), and `report_summary` (overallRiskScore, recommendation, topConcerns). All use `strict: true` and `eager_input_streaming: true`. Exported as `TOOL_DEFINITIONS` for test assertions.
...
- **saferAlternative normalization**: `report_clause` tool schema requires string (not nullable). Claude returns `""` for green clauses when using individual report. Handler converts `""` → `null` to match `ClauseAnalysis` type. Batch green clauses via `report_safe_clauses` set `saferAlternative: null` directly.
...
- **max_tokens estimation**: `Math.min(clauseCount * 300 + 4096, 64000)` — ~300 tokens per clause average (green clauses use ~30 tokens via batch, red/yellow use ~600 with conciseness guidance) + 4K buffer. Cap at 64K (Sonnet 4.6 max output).
