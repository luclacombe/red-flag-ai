...
- **Share + Download buttons:** `AnalysisActions` component. Owner view: share toggle (enables 7-day link + copies URL), stop-sharing button, download PDF, delete. Non-owner view: copy link + download PDF only. Props: `isOwner`, `isPublic`, `shareExpiresAt`. Shows expiry date when shared. Forbidden access shows "This analysis is private" page.
...
- **Auto-deletion cron:** `GET /api/cron/cleanup` runs daily at 02:00 UTC via Vercel Cron. Deletes documents where `COALESCE(expires_at, created_at + 30 days) < NOW()` (CASCADE handles analyses + clauses). `expiresAt` column on documents enables user-controlled renewal. Decrypts `storagePath` to delete from Storage. Deletes `rate_limits` rows >7 days old. Verifies `CRON_SECRET` bearer token. Config in `vercel.json`.
