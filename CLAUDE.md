...
- **Supabase Auth:** Email/password + magic link via `@supabase/ssr`. Three client utilities in `apps/web/src/lib/supabase/`: `client.ts` (browser, `createBrowserClient`), `server.ts` (server components, `createServerClient` + `cookies()`), `middleware.ts` (session refresh, `updateSession()`). Next.js middleware at `apps/web/middleware.ts` calls `updateSession()` on every request. Public routes: `/`, `/login`, `/signup`, `/auth/*`, `/analysis/*`, `/api/*`. Auth pages: unified at `/login` with `mode` state (`"signin" | "signup"`), `/signup` redirects to `/login?mode=signup`. Callback routes: `/auth/callback` (code exchange), `/auth/confirm` (email OTP verification). Custom email templates in `supabase/templates/` (magic-link, confirmation, recovery) with branded dark theme.
