...
- **Streaming** uses tRPC SSE subscriptions via `httpSubscriptionLink` — async generators yielding typed events. SSE subscription is owned by `StreamProvider` (React context at root layout level), not by AnalysisView. This means the subscription survives client-side navigation — navigating to dashboard mid-stream doesn't kill the connection. AnalysisView calls `startStream(id)` and reads raw data from context; shimmer buffering stays local. HistoryView reads live clause counts from the same context for instant dashboard updates.
...
- **Frontend streaming UX:** `StreamProvider` context (`src/context/stream-context.tsx`) at layout level owns the SSE subscription and raw stream state. AnalysisView reads from context via `useStreamContext()`, keeps shimmer buffering local (`displayedClauses` state, `effectiveAnalyzing` memo). `processingStep` derived via `useMemo` from context state (not event-driven). Navigate away mid-stream → subscription survives (layout doesn't unmount). Navigate back → instant display from context (no reconnection). `ProcessingSteps` component shows animated step indicators (connecting → gate → extracting → parsing → preparing → analyzing) before clauses arrive. Minimum 400ms shimmer per clause via `shimmerStartTimes` + `pendingResults` buffering. Each clause transitions: gray → shimmering (min 400ms) → flash → risk color. Green clauses use `GreenClauseCompact` (1-line inline component). Summary skeleton shown between last clause and summary arrival. Auto-scroll follows both analysis cards (`skeletonRef`) AND document panel (scrolls topmost analyzing clause into view). Smart scroll interruption: only flags `userHasInteracted` on upward scroll; re-engages auto-scroll when user scrolls near bottom. Determinate progress bar shows "Analyzed X of N clauses". Processing-to-grid transition uses crossfade (opacity 500ms). HistoryView reads `stream.streamClauses.length` for live clause count on the active-stream item; 2s polling only for other processing items.
- **Side-by-side layout (desktop, ≥ lg/1024px)** available in both streaming and DB render paths. `DocumentPanel` dispatches to `TextDocumentPanel` (block-level highlights, paragraph detection, alternating shades for same-risk clauses). Interaction model: `hoveredClause` (visual highlighting only, no scroll) vs `pinnedClause` (click → scroll other panel with `block: "nearest"`). Derived `activeClause = pinnedClause ?? hoveredClause`. Scroll suppression: `isScrollingRef` flag set on scroll events, cleared after 150ms debounce — `onMouseEnter` is no-op during scroll. `ConnectingLines` draws cubic bezier curves between clause highlights and cards; `docScrollContainer` ref from TextDocumentPanel's inner scrollable div ensures lines track document scroll. `analysis.get` returns `extractedText` + `fileType` from documents table for DB render path.
- **Mobile inline layout (< lg/1024px):** `useIsDesktop()` hook (`src/hooks/use-media-query.ts`) drives layout branching. Below `lg`, the analysis page renders a dark-themed (`dark` prop) single integrated document view with analysis rendered *inside* each clause highlight block as a unified visual unit (no side-by-side panels, no connecting lines). Dark document: `bg-[#0F1629]`, light text (`text-slate-200` clauses, `text-slate-400` plain text), vibrant risk tints (`bg-red-500/10` etc). `TextDocumentPanel` accepts optional `renderClauseSlot` callback — parent injects `InlineAnalysisSection` as last child of each clause highlight div. `InlineAnalysisSection` is a unified component with always-visible elevated header (category + badge + rotating chevron) and body (explanation + safer alt) animated via `grid-template-rows: 0fr → 1fr` for smooth open AND close transitions. Elevation: `bg-white/[0.03]` + `border-white/[0.06]` + multi-layer box-shadow (tight shadow + spread + top-edge highlight). `expandedClauses` state (`Set<number>`) tracks which clauses are expanded. Red/yellow auto-expanded, green collapsed. Tap clause to toggle. Auto-scroll uses `scrollIntoView` on page-level scroll. Works for both streaming and DB render paths, shared/anonymous links.
...
- **Pipeline resumability:** Parse results are cached in `analyses.parsedClauses`. Clause analyses are persisted individually as each `report_clause` tool call completes. On Vercel timeout + reconnect, the pipeline skips completed work and resumes from where it left off. Heartbeat updates `updatedAt` after each yielded event to prevent premature stale detection (30s threshold).
- **Detached pipeline:** The pipeline runs as a detached async operation, decoupled from the SSE connection. Events are forwarded to the client via an in-memory queue. If the client disconnects (navigates away), the pipeline keeps running and persisting results to DB. On reconnect, the "processing, not stale" polling path picks up from DB. Safety timeout: 5 minutes for queue forwarding.
...
- **Response language selection:** Users choose what language Claude writes explanations in, independent of the document language. 16 entries in `SUPPORTED_LANGUAGES` (15 languages + `"auto"`). `AUTO_LANGUAGE_CODE = "auto"` means "use document's detected language" — resolved in orchestrator via `params.language`. Default is `"auto"` (document's original language). `LanguageSelector` label: "Risk analysis in". Info tooltip explains: controls explanation language, not document translation; safer alternatives stay in original language. `responseLanguage` stored on analysis record, threaded through orchestrator → combined analysis → summary.
- **Shareable URLs:** Analyses are **private by default** (`isPublic = false`). Owners enable sharing via `analysis.toggleShare` mutation, which sets `isPublic = true` and `shareExpiresAt = now + 7 days`. Access check: owner always has access, anonymous uploads (no userId) accessible by UUID, others need `isPublic = true && shareExpiresAt > now()`. `SHARE_LINK_EXPIRY_DAYS` constant in `@redflag/shared`. Dynamic OG meta tags and OG image only generated for shared/anonymous analyses. Non-shared analyses show generic metadata. `analysis.get` returns `isOwner: boolean` for frontend to conditionally render share toggle vs read-only view.
...
- **Share + Download buttons:** `AnalysisActions` component. Props: `analysisId`, `isOwner`, `isPublic`, `shareExpiresAt`, `isAuthenticated`. Authenticated owner: share toggle (enables 7-day link + copies URL), stop-sharing button, download PDF, delete. Non-owner: copy link + download PDF only. Anonymous (`!isAuthenticated`): download PDF only + "Sign in to share" CTA linking to `/login`. Shows expiry date when shared. Forbidden access shows "This analysis is private" page.
- **Supabase Auth:** Email/password + magic link + OAuth (Google, Microsoft/Azure, GitHub) via `@supabase/ssr`. Three client utilities in `apps/web/src/lib/supabase/`: `client.ts` (browser, `createBrowserClient`), `server.ts` (server components, `createServerClient` + `cookies()`), `middleware.ts` (session refresh, `updateSession()`). Next.js middleware at `apps/web/middleware.ts` calls `updateSession()` on every request. Public routes: `/`, `/login`, `/signup`, `/auth/*`, `/analysis/*`, `/api/*`. Auth pages: unified at `/login` with `mode` state (`"signin" | "signup"`), `/signup` redirects to `/login?mode=signup`. OAuth buttons (Google, Microsoft, GitHub) call `signInWithOAuth()` → redirect to provider → callback → dashboard. Callback routes: `/auth/callback` (code exchange for both OAuth and magic link), `/auth/confirm` (email OTP verification). Custom email templates in `supabase/templates/` (magic-link, confirmation, recovery) with branded dark theme. OAuth provider config in `supabase/config.toml` (`[auth.external.google]`, `[auth.external.azure]`, `[auth.external.github]`). Provider credentials set via env vars in Supabase dashboard (production) or config.toml env() refs (local).
...
- **Auth-aware rate limiting:** `checkRateLimit(identifier, isAuthenticated)` — authenticated users get 3/day (by userId), anonymous get 1/day (by IP). Constants: `RATE_LIMIT_PER_DAY` (1), `RATE_LIMIT_AUTH_PER_DAY` (3) in `@redflag/shared`.
...
- **NavBar auth state:** Client component with `useEffect` for `getUser()` + `onAuthStateChange`. Authenticated: dropdown menu with Dashboard link, Sign out, Delete account (with `ConfirmDialog`). Unauthenticated: "Sign in" link.
- **Dashboard:** `/dashboard` page (protected by middleware redirect). Authenticated users on `/login` or `/signup` are redirected here. `HistoryView` client component uses `trpc.analysis.list.useInfiniteQuery` with cursor-based pagination. Compact `UploadZone` at top for quick uploads (redirects to analysis on success). Full `UploadZone` shown as empty state. Each item shows risk score, document name, contract type badge, recommendation badge, date, share status, and expiry warning (≤7 days). Action row below each complete analysis: Rename (inline edit), Download, Rerun, Rerun in... (language dropdown with outside-click-to-close), Share/Unshare (copies URL to clipboard), Renew (when ≤7 days), Delete. All mutations show toast notifications (success + error) via `sonner`. NavBar link renamed from "History" to "Dashboard" with `LayoutDashboard` icon.
- **Toast notifications:** `sonner` library. `<Toaster>` in root layout (`layout.tsx`) with dark theme matching `#131B2E` surface color. Used for mutation feedback throughout the dashboard (delete, rename, share, rerun, renew, upload).
...
- **Auto-deletion cron:** `GET /api/cron/cleanup` runs daily at 02:00 UTC via Vercel Cron. Deletes documents where `COALESCE(expires_at, created_at + 30 days) < NOW()` (CASCADE handles analyses + clauses). Also deletes anonymous documents (`userId IS NULL`) older than 24 hours. `expiresAt` column on documents enables user-controlled renewal. Decrypts `storagePath` to delete from Storage. Deletes `rate_limits` rows >7 days old. Verifies `CRON_SECRET` bearer token. Config in `vercel.json`.
- **Anonymous data cleanup:** Two-layer approach. Layer 1: `POST /api/cleanup` accepts `{ analysisId }` via `sendBeacon` on `visibilitychange` (tab close/switch). Only deletes documents with `userId = null`. Layer 2: cron fallback deletes anonymous documents older than 24h (catches missed sendBeacon calls).
- **Document renewal:** `analysis.renew` mutation resets the auto-deletion timer by setting `documents.expiresAt = now + 30 days`. Dashboard shows "Auto-deletes in X days" when ≤7 days remaining, with a "Renew" button.
- **Analysis rerun:** `analysis.rerun` mutation creates a new analysis record for the same document, optionally with a different `responseLanguage`. User redirected to the new analysis page where SSE subscription triggers the pipeline. Original analysis preserved.
- **Document rename:** `analysis.rename` mutation encrypts and updates `documents.filename`. Inline edit in dashboard.
...
| sonner | latest | Toast notifications (dark themed, bottom-right) |
...
- **Migrations:** `supabase/migrations/` — 6 files: pgvector extension, consolidated schema (all 5 tables), RLS policies, pipeline_metrics table, share links, document expires_at
