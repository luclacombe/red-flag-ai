...
- [ ] Ensure `/analysis/[id]` pages work without authentication (they already do in MVP — verify this continues to work after Phase 4 adds auth)
- [ ] Add dynamic Open Graph meta tags to `apps/web/app/analysis/[id]/page.tsx`:
...
- [ ] Create a server component that fetches analysis data for metadata generation (Next.js `generateMetadata`)

#### 3.2 — Share functionality
- [ ] Add "Share" button to the analysis page header (after analysis completes)
- [ ] On click: copy URL to clipboard, show brief toast/confirmation "Link copied!"
- [ ] Use `navigator.clipboard.writeText()` — no library needed

#### 3.3 — PDF report generation
- [ ] Install `@react-pdf/renderer`: `pnpm add @react-pdf/renderer --filter @redflag/web`
- [ ] Create `apps/web/app/api/report/[id]/route.ts` — a GET route handler that:
...
- [ ] Add "Download PDF" button to the analysis page (next to Share button)
- [ ] Link to `/api/report/[id]` — triggers browser download

#### 3.4 — OG Image generation (optional, skip if time-constrained)
- [ ] Use Vercel OG (`@vercel/og`) or a static template image
- [ ] Generate dynamic OG image showing: risk score gauge + recommendation badge
- [ ] Route: `apps/web/app/api/og/[id]/route.ts`

#### 3.5 — Tests
- [ ] Unit test: `generateMetadata` returns correct OG tags for a given analysis
- [ ] Unit test: PDF report route returns valid PDF buffer with correct headers
- [ ] Unit test: Share URL copy (if testable — may be integration-only)

#### 3.6 — Documentation
- [ ] Update CLAUDE.md: add `@react-pdf/renderer` dependency, OG image route, report route
- [ ] Update PROJECT.md: add sharing/export to feature list

### Exit Criteria
- [ ] Quality gate passes: `pnpm turbo lint type-check test build`
- [ ] Analysis page has rich link previews when shared (test with OpenGraph debugger)
- [ ] "Share" button copies URL to clipboard
- [ ] "Download PDF" button downloads a formatted report
- [ ] Documentation updated
